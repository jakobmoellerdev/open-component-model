...
import (
	...
	"ocm.software/open-component-model/bindings/go/oci/resolver"
...
func init() {
	...
	if err := manager.RegisterInternalComponentVersionRepositoryPlugin(
		scheme,
		&Plugin{scheme: scheme, memory: memory.NewInMemory()},
		&v1.OCIRepository{},
	); err != nil {
		panic(err)
	}
...
type Plugin struct {
	...
	memory memory.LocalDescriptorMemory
}
...
func createRepository(spec *v1.OCIRepository, credentials manager.Attributes, p Plugin) (oci.ComponentVersionRepository, error) {
	...
	url, err := runtime.ParseURLAndAllowNoScheme(rurl)
	if err != nil {
		return nil, fmt.Errorf("invalid URL %q: %w", rurl, err)
	}
	urlString := url.Host + url.Path

	urlResolver := resolver.NewURLPathResolver(urlString)
	...
	repo, err := oci.NewRepository(
		...
		oci.WithLocalManifestBlobMemory(p.memory),
	)
	return repo, err
}
