...
func TypedToVal(val interface{}, schema *jsonschema.Schema) ref.Val {
	if val == nil {
	...
	case reflect.Slice:
		...
		return &typedList{value: v, itemsSchema: schema.Items}
	case reflect.Map:
		return &typedMap{value: v, valuesSchema: schema.AdditionalProperties}
	case reflect.Struct:
		...
		return &typedStruct{
			value: v,
			propSchema: func(key string) (*jsonschema.Schema, bool) {
				if schema, ok := schema.Properties.Get(key); ok {
					return schema, true
				}
				return nil, false
			},
...
func (s *typedStruct) Equal(other ref.Val) ref.Val {
	...
	if ok {
		return types.Bool(reflect.DeepEqual(s.value.Interface(), otherStruct.value.Interface()))
	}
