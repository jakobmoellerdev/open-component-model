...
type envOptions struct {
	...
	typedResources map[string]*invopop.Schema
...
func WithTypedResources(schemas map[string]*invopop.Schema) EnvOption {
	return func(opts *envOptions) {
...
func DefaultEnvironment(options ...EnvOption) (*cel.Env, error) {
	...
	if len(opts.typedResources) > 0 {
		...
		if len(declTypes) > 0 {
			baseProvider := jsonschema.NewDeclTypeProvider(declTypes...)
...
func TypedEnvironment(schemas map[string]*invopop.Schema) (*cel.Env, error) {
