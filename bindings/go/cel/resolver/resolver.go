...
func (summary *ResolutionSummary) Error() error {
	return errors.Join(summary.Errors...)
}

// Resolver handles the resolution of CEL expressions in Kubernetes resources.
type Resolver struct {
	// The original object to be resolved. In kro, this will typically
	// be a Kubernetes object with some fields containing CEL expressions.
...
func NewResolver(object map[string]interface{}, data map[string]interface{}) *Resolver {
	return &Resolver{
		object: object,
		data:   data,
	}
...
func (r *Resolver) Resolve(expressions []parser.FieldDescriptor) ResolutionSummary {
...
// UpsertValueAtPath sets a value in the object using the fieldpath parser.
...
// the object. It returns a ResolutionResult containing information about the
// resolution process
func (r *Resolver) resolveField(field parser.FieldDescriptor) ResolutionResult {
...
// getValueFromPath retrieves a value from the object using a dot separated path.
...
// setValueAtPath sets a value in the object using a dot-separated path.
func (r *Resolver) setValueAtPath(path string, value interface{}) error {
	...
	var parent interface{} = r.object
	var current interface{} = r.object
