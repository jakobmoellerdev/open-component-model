...
func TestClientGet_Auth(t *testing.T) {
	...
	t.Run("username+password yields Basic auth", func(t *testing.T) {
		...
		t.Cleanup(srv.Close)
		resp, err := NewClient(srv.Client()).Get(context.Background(), srv.URL, "", typedCreds("__token__", "pypi-xxx", ""))
		require.NoError(t, err)
		_ = resp.Body.Close()
		assert.Equal(t, "__token__", user)
		assert.Equal(t, "pypi-xxx", pass)
	})
	...
	t.Run("password-only credentials are rejected", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}))
		t.Cleanup(srv.Close)
		_, err := NewClient(srv.Client()).Get(context.Background(), srv.URL, "", typedCreds("", "s3cr3t", ""))
