...
//   - Raw → Typed: unmarshals YAML from raw.Data into the typed object (if the type is registered).
//   - Typed → Raw: marshals the object to JSON, applies canonicalization, and stores the result in Raw.Data. (See Raw.UnmarshalJSON)
...
func (r *Scheme) Convert(from Typed, into Typed) error {
	...
	if rawFrom, ok := from.(*Raw); ok {
		...
		if err := yaml.Unmarshal(rawFrom.Data, into); err != nil {
	...
	// Typed → Raw
	if rawInto, ok := into.(*Raw); ok {
		// If 'from' is a *Raw (shouldn't happen due to earlier check, but kept for safety)
		if rawFrom, ok := from.(*Raw); ok {
			rawFrom.DeepCopyInto(rawInto)
			return nil
		}
